...
import { createElementCaptureMode } from "../../../../src/capture/modes/element.js";
...
function createMocks() {
	return {
		...
		mockDoc: {
			...
			createElement: (tag) => ({
				tagName: tag.toUpperCase(), id: "", style: {}, textContent: "",
...
describe("createElementCaptureMode", () => {
	test("returns an object with run method", () => {
		const m = createMocks();
		const mode = createElementCaptureMode({
	...
	test("releases session in finally block", async () => {
		const m = createMocks();
